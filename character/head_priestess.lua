...
function HeadPriestess:on_interact(interactor_name)
  if not self.control.data.created_character then
    ...
    if not self.control.data.invited_to_training then
      local dialogue = {
        start = {
          text = "Perhaps you will find useful to visit the training grounds. Just follow the road and talk to the priestesses. All of them will have something to give you.",
          go_to = 'end',
          callback = function()
            self.control.data.invited_to_training = true
          end
        }
      }
      sfml_dialogue(dialogue)
